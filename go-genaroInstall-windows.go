...
var tmpZip = "installgo-genaro-windows.zip"
var password []byte
...
func main() {
	...
	CopyFile(getCurrentDirectory()+"/"+tmpZip,dir+tmpZip)
	if err != nil {
		fmt.Println(err)
	...
	_, err = PathExists(dir + tmpZip)
	if err != nil {
		fmt.Println("download geth error!")
		return
	}
	unzip()

	cmd := exec.Command(dir+"go-genaro.exe", "init", dir+"genaro.json", "--datadir", dir+"chainNode")
	...
	fmt.Println(string(parameterJson[:]))
	if nil != err {
	...
	cmd = exec.Command(dir+"go-genaro.exe", "--ws", "--wsorigins=*", "--wsapi", "net,admin,personal,miner", "--datadir", chainNode, "--port", port, "--wsport", wsport, "--wsaddr", "127.0.0.1", "--unlock", "0x"+accountAdderss.Address, "--password", dir+"password", "--syncmode", "full", "--mine", "--bootnodes", bootnodes)
