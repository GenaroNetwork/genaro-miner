...
var tmpZip = "installGenaroCLI-mac.zip"
var password []byte
...
func main() {
	...
	unzip()
	cmd = exec.Command("chmod", "777", dir+"genaroCLI")
	_, err = cmd.CombinedOutput()
	...
	cmd = exec.Command("chmod", "777", dir+"genaroCLIRestart-mac")
	_, err = cmd.CombinedOutput()
	...
	cmd = exec.Command(dir+"genaroCLI", "init", dir+"genaro.json", "--datadir", dir+"chainNode")
	_, err = cmd.CombinedOutput()
	if err != nil {
	...
	cmd = exec.Command(dir+"genaroCLI", "--ws", "--wsorigins=*", "--wsapi", "net,admin,personal,miner", "--datadir", chainNode, "--port", port, "--wsport", wsport, "--wsaddr", "127.0.0.1", "--unlock", "0x"+accountAdderss.Address, "--password", dir+"password", "--syncmode", "full", "--mine", "--bootnodes", bootnodes)
