...
var tmpZip = "installgo-genaro-linux.zip"
var password []byte
...
func main() {
	...
	cmd := exec.Command("cp", "installgo-genaro-linux.zip", dir)
	...
	unzip()
	cmd = exec.Command("chmod", "777", dir+"go-genaro")
	_, err = cmd.CombinedOutput()
	...
	cmd = exec.Command("chmod", "777", dir+"go-genaroRestart-linux")
	_, err = cmd.CombinedOutput()
	...
	cmd = exec.Command(dir+"go-genaro", "init", dir+"genaro.json", "--datadir", dir+"chainNode")
	_, err = cmd.CombinedOutput()
	if err != nil {
	...
	cmd = exec.Command(dir+"go-genaro", "--ws", "--wsorigins=*", "--wsapi", "net,admin,personal,miner", "--datadir", chainNode, "--port", port, "--wsport", wsport, "--wsaddr", "127.0.0.1", "--unlock", "0x"+accountAdderss.Address, "--password", dir+"password", "--syncmode", "full", "--mine", "--bootnodes", bootnodes)
