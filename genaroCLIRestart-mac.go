...
func main() {
	...
	cmd := exec.Command(parameter.Dir+"genaroCLI", "--ws", "--wsorigins=*", "--wsapi", "net,admin,personal,miner", "--datadir", parameter.ChainNode, "--port", parameter.Port, "--wsport", parameter.Wsport, "--wsaddr", "127.0.0.1", "--unlock", "0x"+parameter.Address, "--password", parameter.Dir+"password", "--syncmode", "full", "--mine", "--bootnodes", parameter.Bootnodes)
